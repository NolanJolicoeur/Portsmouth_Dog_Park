...
server <- shinyServer(function(input, output) {
    output$FileNames <- renderText(log)
    observeEvent(input$Click1,{
        ...
        process <- function(data1){
            ...
            return(df)}
        log <- readRDS('~/Dog_Park/File_Log.R')
        files <- c(df)
        NewFile <- process(NewFile)
        df3 <- readRDS('~/Dog_Park/Data_Log.R')
        NewFile = sqldf("SELECT * FROM NewFile UNION SELECT * FROM df3")
        saveRDS(NewFile, 'Data_Log.R')
        output$FileNames <- renderText(file) 
    })
    observeEvent(input$Click, {
        ...
        df3 <- readRDS('~/Dog_Park/Dog_Park_Test_App/Data_Log.R')
